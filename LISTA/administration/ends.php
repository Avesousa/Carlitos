<?php 
    ...
    $all = "SELECT * FROM `enviodomicilio` WHERE `motivo` != '' AND `motivo` != ' ' AND `disponible` = 1 ORDER BY id";
    $page_all = "SELECT * FROM enviodomicilio WHERE `motivo` != '' AND `motivo` != ' ' AND `disponible` = 1 ORDER BY id DESC";

    if(!empty($search)){
        $all = "SELECT `id`,`documento`,`cliente`,`direccion`,`telefono`,`mail`,`creacion`,`monto` 
                FROM `enviodomicilio` 
                WHERE (id LIKE '%$search%' OR documento LIKE '%$search%' OR cliente LIKE '%$search%' OR direccion LIKE '%$search%') AND `disponible` = 1 AND `motivo` != '' AND `motivo` != ' '
                ORDER BY id";
...
<body>
    ...
    <div id="all" class="container scrollflow -slide-top -opacity">
        <div class="row">
            <form action="ends.php" method="get" class="adminSearch">
                ...
                <div class="row">
                    ...
                    <?php
                        ...
                        if($pagination > 1){
                            ...
                            if($page > 1){
                                echo "<a class='adminPage' href='ends.php?page=1&search=".$search."'>Primera</a>";
                                echo "<a class='adminPage' href='ends.php?page=".($page - 1)."&search=".$search."'><<</a>";
                            }
                                echo "<a class='adminPage adminPageSelected' href='ends.php?page=".$loop."&search=".$search."'>".$loop."</a>";
    
                            for($i=$loop+1; $i<=$loop_end; $i++){     
                                echo "<a class='adminPage' href='ends.php?page=".$i."&search=".$search."'>".$i."</a>";
                            }
                            if($page <= $loop_end){
                                echo "<a class='adminPage' href='ends.php?page=".($page + 1)."&search=".$search."'>>></a>";
                                echo "<a class='adminPage' href='ends.php?page=".$pagination."&search=".$search."'>Última</a>";
                            }
                        } else{
                            echo "<a class='adminPage adminPageSelected' href='ends.php?page=1&search=".$search."'>1</a>";
                        }
                    ?>
